    ...
    return (
      <div className={`chain-selector relative ${className}`}>
        <button
          ...
          className={`flex items-center gap-1.5 px-2 sm:px-2.5 py-2 sm:py-1.5 rounded-md border transition-all duration-200 gpu-accelerate min-h-[40px] min-w-[60px] sm:min-w-[70px]
        ...
        >
          <span className="text-base sm:text-sm">{isSolanaMode ? SOLANA_CHAIN.icon : currentChain.icon}</span>
          <span className={`text-xs sm:text-xs font-bold ${isSolanaMode ? SOLANA_CHAIN.textColor : currentChain.textColor}`}>
        ...
        {isOpen && (
          <>
            ...
            />
            <div className="fixed left-2 right-2 top-16 sm:absolute sm:top-full sm:left-0 sm:right-auto mt-1 sm:w-52 w-auto bg-black/98 border border-cyan-500/40 rounded-xl shadow-2xl backdrop-blur-md z-[10000] overflow-hidden gpu-accelerate">
              <div className="p-2 sm:p-1 max-h-[70vh] overflow-y-auto">
                <div className="px-3 py-2 text-[10px] uppercase tracking-widest text-cyan-500/60 font-mono">
                  EVM Chains
                </div>
                {SUPPORTED_CHAINS.map((chain) => (
                  <button
                    ...
                    disabled={isPending}
                    className={`w-full flex items-center gap-3 px-3 py-3 sm:py-2 rounded-lg transition-all duration-150 min-h-[48px] sm:min-h-[40px]
                  ...
                  >
                    <span className="text-xl sm:text-base">{chain.icon}</span>
                    <span className={`text-sm sm:text-xs font-medium ${chain.textColor}`}>{chain.name}</span>
                ...
                <div className="px-3 py-2 text-[10px] uppercase tracking-widest text-purple-500/60 font-mono">
                  Non-EVM
                </div>
                <button
                ...
                >
                  <span className="text-xl sm:text-base">{SOLANA_CHAIN.icon}</span>
                  <span className={`text-sm sm:text-xs font-medium ${SOLANA_CHAIN.textColor}`}>{SOLANA_CHAIN.name}</span>
                  <ExternalLink size={12} className="ml-auto text-purple-400/60" />
                </button>
              </div>
            </div>
          </>
